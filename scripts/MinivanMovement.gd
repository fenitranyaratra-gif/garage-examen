...
# Ajoutez cette méthode à la fin de votre script Voiture (celui avec extends CharacterBody2D)

func repair():
	print("La voiture a reçu l'ordre de réparation")
	
	# Rechercher le nœud parent (Civic) qui contient la logique de réparation
	var parent = get_parent()
	if parent and parent.has_method("_on_repair_button_pressed"):
		print("Appel de la réparation sur le parent")
		parent._on_repair_button_pressed()
	elif get_tree().get_nodes_in_group("voiture_ui").size() > 0:
		# Alternative : si vous avez un groupe pour les UIs de voiture
		for ui in get_tree().get_nodes_in_group("voiture_ui"):
			if ui.has_method("_on_repair_button_pressed"):
				ui._on_repair_button_pressed()
				break
	else:
		print("Erreur : Impossible de trouver la logique de réparation")
